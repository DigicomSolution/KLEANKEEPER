...
class CaseStudyController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->model = new CaseStudy;
        $this->route = 'admin.case-studies';
        $this->views = 'admin.case_studies';
    ...
    public function store(HttpRequest $r)
    {
        ...
        return Redirect::to(url('admin/case-studies/edit', array('id'=>encrypt($this->model->id))))->withSuccess('Case study successfully saved!'); 
    }
        ...
        if($obj = $this->model->find($id)){
            ...
            if($new_services)
            {
                foreach ($new_services as $key => $service) {
                    $check_exist = DB::table('case_study_services')->where('case_study_id', $obj->id)->where('service_id', (int)$service)->first();
                    if($check_exist)
            ...
            return Redirect::to(url('admin/case-studies/edit', array('id'=>encrypt($obj->id))))->withSuccess('Case study details successfully updated!');
        } else {
